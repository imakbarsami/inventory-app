                    ...
                    <form method="POST" action="{{ route('products.store') }}">
                        @csrf <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" name="sell_price" id="sell_price" value="{{ old('sell_price') }}" required
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                @error('sell_price')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                <label for="stock" class="block text-sm font-medium text-gray-700">Opening Stock (Quantity)</label>
